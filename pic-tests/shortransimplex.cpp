        ...
        for (unsigned int _j = 0; _j < shortList[i].size(); ++_j) {
            ...
            if (tmp < -1e-15) {
                ...
                if (newlyList.size() > __k__) {
                    //mexPrintf("This time discovered %d items before iterating %f percent.\n", __k__, __p__);
                    return;
                }
            }
        }
...
void core() {
    ...
    // mexPrintf("%.4fs have been used on build short list.\n", elapsed_secs);
    // begin = clock();
    ...
    while (true) {
        ...
        if (shortSearch) {
            if (!mostnegativeShort(nr, nc)) {
                shortSearch = false;
                mexPrintf("Phase 3 ends at iteration %d.\n", itr);
